...
interface TaskStoreWithRx extends TaskStore {
  ...
  silentUpdate: (partial: Partial<Pick<TaskStore, 'tasks' | 'tasksLoaded' | 'tasksNeedingRepaint'>>) => void;
}
...
export const useTaskStore = create<TaskStore>((set, get) => ({
  ...
  loadAllTasks: async (userSession: UserSession) => {
    ...
    const tasksArray = allTasks.map((doc: RxDocument<TaskDocument>) => doc.toJSON() as TaskDocument);
